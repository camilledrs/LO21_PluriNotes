...
void Relation::suppCouple(const Couple& c)
{
	...
	else 
	{
		...
		if (note1->getActive() == false){
			...
			if(it==end) //la note n'est plus en couple nulle part
			{
				int reponse= new QMessageBox::question(???,"Supprimer de note", "La note " note1->getId() " est archivée et n'est plus référencée, voulez-vous  la supprimer ?", QMessageBox::Yes | QMessageBox::No);
				if(reponse == QMessageBox::Yes)
					delete note1;
		 		// fait apparaitre une fenêtre de dialogue avec l’utilisateur
			}
